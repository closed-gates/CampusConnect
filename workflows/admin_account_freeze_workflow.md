...
5. While a user is signed in, `useFreezeWatcher` subscribes to the user's WebSocket topic. A freeze event opens `FrozenAccountDialog` immediately. It also checks `GET /api/admin/account-freezes/me` on mount, reconnect, and every five seconds so a missed event cannot leave a frozen session active.
6. On login, `AuthService` rejects an inactive account with the `ACCOUNT_FROZEN` response marker. `useLoginController` converts that response into `FrozenAccountDialog`; no token is issued.
7. Unfreezing restores `app_users.active`, allowing the account to authenticate again with its existing credentials and data.
