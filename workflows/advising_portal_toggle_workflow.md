...
StudentPanel delegates all status display to RegistrationView. Its registrationController polls GET /api/registration/window/{studentId} every five seconds and on tab focus/visibility. RegistrationService reads the persisted global portal flag and returns portalOpen, portalMessage, and effective open status; student priority tiers remain informational metadata. The same response controls the banner and Register buttons. Opening the portal grants advising access to every student, while closing it blocks student registration. Failed requests pause registration and display a reconnecting state rather than assuming open. The polling loop avoids concurrent requests and ignores responses after unmount.
